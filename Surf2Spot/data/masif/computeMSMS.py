...
def computeMSMS(pdb_file,  
                protonate=True, 
                one_cavity=None, 
                msms_bin="/data/zaw/development/HSfilter/get_ply/masif/extra_dependence/msms/msms.x86_64Linux2.2.6.1",
                workdir=".", probe = '5.0'):
    ...
    if one_cavity is not None:
        args = [msms_bin, "-density", "1.0", "-hdensity", "1.0", "-probe", probe,
                "-one_cavity", str(1), str(one_cavity),\
                "-if",out_xyzrn,"-of",file_base, "-af", file_base]
    else:
        args = [msms_bin, "-density", "1.0", "-hdensity", "1.0", "-probe",\
                    probe, "-all_components", "-if",out_xyzrn,"-of",file_base, "-af", file_base]  #probe org 1.5
