...
def get_embeddings( seq_path, 
                   ...
                   max_batch=1000 # max number of sequences per single batch
                   ):
...
def run_prottrans(seq_path, emb_path):   
    seq_path   = Path( seq_path )
    emb_path   = Path( emb_path)
    model_dir  = Path( os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model_emb/prot_t5_xl_half_uniref50-enc') ) 
