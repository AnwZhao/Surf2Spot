...
def align_data(pdb_file, point_cloud_data, amino_acid_data,  progen2_data, dist_arr):
    ...
    #print('++++matched_amino_acid_features++++',matched_amino_acid_features.shape)
...
def parse_all_protein_data(pdb_files, protein_files, amino_acid_atom_files, emb_path):
    ...
    f.close()  
    return data_list, pdb_file_list
...
class DGCNNConfig(PretrainedConfig):
    model_type = "dgcnn"

    def __init__(self, k=20, pc_dim=39, output_dim=1, output_progen2_dim=16, output_dist_arr_dim=32, **kwargs):
        super().__init__(**kwargs)
        self.k = k
        self.pc_dim = pc_dim
        self.output_dim = output_dim
        self.output_progen2_dim = output_progen2_dim
        self.output_dist_arr_dim = output_dist_arr_dim

        
class HuggingFaceDGCNN(PreTrainedModel):
    config_class = DGCNNConfig
    ...
    def __init__(self, config):
        ...
        self.model = DGCNN(
            ...
            output_progen2_dim=config.output_progen2_dim,
            output_dist_arr_dim=config.output_dist_arr_dim,
        )
...
def main():
    ...
    parser.add_argument('--model', type = str, default = 'model/proHS/model.pt',
                        help='model path')
    parser.add_argument('--threshold', type = float, default = 0.4,
                        help='threshold')
    parser.add_argument('--emb', type = str, required= True, help='progen2 embedding path')
    ...
    data_list, pdb_file_list = parse_all_protein_data(pdb_files, protein_files, amino_acid_atom_files, emb_path)
    print('--------------------------')

    protein_files = []
    for f in pdb_files:
        protein_files.append(f.split('.')[0] + '_all_5.0_filtered.ply')
    ...
    model = HuggingFaceDGCNN.from_pretrained(args.model)
    model.to(device)
    model.eval()
